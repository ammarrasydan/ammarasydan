...
// Get theme options values.
$thinkup_general_boxlayout = thinkup_var ( 'thinkup_general_boxlayout' );
...
/* ----------------------------------------------------------------------------------
	BACK UP OPTIONS TO PAGE "SENTO CREATED CONTENT BACKUP"
---------------------------------------------------------------------------------- */
...
global $wp_customize;
	...
	if ( $thinkup_general_backupswitch == '1' ) {
		...
		$output_header  .= esc_html__( 'This page contains a backup of content created by the Sento WordPress Theme. ', 'sento' );
		$output_header  .= esc_html__( 'The purpose for the backup is to prevent content loss on theme switch.', 'sento' );
		$output_header  .= esc_html__( 'When a user switches themes this content will still be available to the user when setting up their site on the new theme.', 'sento' ) . "\n";
		...
		// Backup page setup.
		$postarray['post_title']     = 'Sento Created Content Backup'; // translate ok.
		$postarray['post_type']      = 'page';
		$postarray['post_status']    = 'private';
		$postarray['comment_status'] = 'closed';
		$page                        = get_page_by_title( 'Sento Created Content Backup' );
